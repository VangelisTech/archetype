...
class MissionControlCatalog:
    ...
    def __init__(self, profiles: ExecutionProfileCatalog) -> None:
        ...
        self._run_execution: Callable[[str, str, str], object] | None = None
    ...
    def bind_run_execution(self, resolver: Callable[[str, str, str], object]) -> None:
        """Bind the host-owned pinned-profile execution resolver.

        Concrete composition (sandbox backends, drivers, credentials) stays
        with the host. The resolver receives the exact pinned
        ``(profile_id, version, digest)`` recorded on a durable MissionRun.
        """

        if not callable(resolver):
            raise TypeError("run execution resolver must be callable")
        self._run_execution = resolver

    def run_execution_config(self, profile_id: str, version: str, digest: str) -> object:
        """Resolve one pinned profile to host execution config or fail closed."""

        if self._run_execution is None:
            raise RuntimeError(
                "mission-run execution is not bound for this host; "
                "bind a profile execution resolver on the mission-control capability"
            )
        return self._run_execution(profile_id, version, digest)
