...
async def _record_terminal(
    ...
    *,
    ...
    if status is RunStatus.SUCCEEDED and (rollout is None or evaluation is None):
        raise ValueError("SUCCEEDED requires rollout and evaluation")
    ...
    async with world_registry.operation(lab_world_id) as lab:
        ...
        if status is RunStatus.SUCCEEDED and improved and head_entity_id is not None:
            assert evaluation is not None
            await mutation._update_entity_locked(
                lab,
                head_entity_id,
