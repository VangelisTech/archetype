...
def main() -> int:
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Audit every Python file under src/ — the explicit spelling of the "
        "no-argument default, for call sites (CI, Makefile) where whole-tree "
    ...
    if args.all and args.files:
        parser.error("--all audits the whole source tree; do not also pass explicit files")

    python_files, errors = resolve_files([] if args.all else args.files)
