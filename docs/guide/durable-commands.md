...
A directly constructed value or type reference also retains an immutable,
...
loaded schema-identical definitions from multiple modules. Canonical JSON and
JSON Schema omit the affinity. Durable decoding therefore remains portable
and resolves only from the name and schema fingerprint.
