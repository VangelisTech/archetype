# Legacy mission attempt kernel

**Document type:** Normative compatibility record for an internal subsystem.

> **This is not Agent Missions V1.**
>
> This page describes the claim-, fence-, and artifact-finalization stack
> retained under `archetype.app.missions` and `archetype.app.sandboxes`.
> The public software-factory workflow does not call this stack. Start with
> [Agent Missions V1](agent-missions.md) for the current authoring and
> execution contract.
...
Within this compatibility subsystem, the specification owns when its legacy
single-row task cursor may advance. It does not own or override V1 task-entity
transitions. Sandbox execution, provider credentials, artifact publication,
and repository automation remain separate capabilities.
...
## 7. Superseded package-map record

> **Historical only.**
>
> This section records the package inventory attached to the legacy kernel.
> It is no longer an architecture source and includes dependency claims that
> Agent Missions V1 has superseded. Use
> [Agent Missions V1, section 9](agent-missions.md#9-family-direction-after-v1)
> and [Application Architecture](application-architecture.md) for current
> direction and machine-enforced edges.

The superseded inventory remains available in Git history. No current package
placement, dependency edge, implementation stage, or compatibility promise
