...
## Capturing trajectories from Claude Code

If you want trajectories *generated by* an agent rather than ingested from
elsewhere, see [AgentPrism Capture](agentprism.md): it adds a
`ClaudeCodeProcessor` (a `daft.cls` wrapping the `claude` CLI's stream-json
stdout) plus an AgentPrism-compatible JSON exporter so the captured runs feed
directly into the labeling pipeline above.
