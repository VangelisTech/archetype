...
**Status:** The boundary and migration order are ratified. The generic
catalog, canonical hosted Physical-AI data contract, opt-in Mission
author/critic integrations, and family-owned hosted Physical-AI Activity
choreography (local and Modal parity) are implemented. Final topology
consolidation remains a separate slice. Each implementation slice must land
its executable oracle with the behavior it enables.
