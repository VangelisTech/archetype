...
| AuditLog | **Advisory durability class** in v0.3: bounded batches append to a dedicated Iceberg `audit_rows` table, and `_emit` never raises (an audit failure must not fail the gated operation). Facts and receipts remain the durable evidence boundary. Compaction is a separate storage-maintenance feature. |
