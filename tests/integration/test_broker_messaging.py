...
from archetype.dsl import World, behavior, Inbox
...
class TestCommandBroker:
    ...
    async def test_bulk_enqueue(self):
        ...
        commands = [
            Command(type=CommandType.MESSAGE, payload={"idx": i})
            for i in range(100)
        ]
...
class TestMessaging:
    ...
    async def test_targeted_message(self, tmp_path):
        ...
        async with World("targeted", storage=tmp_path / "data") as world:
            ...
            bystander = await world.find_one(lambda a: a.agent.name == "Bystander")
