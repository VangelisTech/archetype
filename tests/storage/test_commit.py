...
async def test_publish_registers_once_settles_staged_commands_and_delegates_visibility() -> None:
    ...
    next_context = await coordinator.begin_tick("world", "run", 5)
    await coordinator.publish_tick("world", "run", 5, next_context, [signature])
    assert len(catalog.registered) == 1
...
def test_staging_rejects_multiple_lease_owners_for_one_tick() -> None:
    coordinator = CatalogCommitCoordinator(RecordingCatalog(), epoch=1)
...
async def test_failed_publish_keeps_staged_command_retryable() -> None:
    ...
    coordinator = CatalogCommitCoordinator(catalog, epoch=3)
    coordinator.stage_command(6, "worker-a", "command-1")
    context = await coordinator.begin_tick("world", "run", 6)

    with pytest.raises(RuntimeError, match="injected publish failure"):
        await coordinator.publish_tick("world", "run", 6, context, [])
...
async def test_bound_coordinator_rejects_mismatched_begin_identity(
    world_id: str,
    run_id: str,
) -> None:
    coordinator = CatalogCommitCoordinator.bound(
    ...
    with pytest.raises(ValueError, match="commit coordinator identity mismatch"):
        await coordinator.begin_tick(world_id, run_id, 0)
...
async def test_bound_coordinator_rejects_mismatched_publish_before_catalog_writes(
    world_id: str,
    run_id: str,
) -> None:
    catalog = RecordingCatalog()
    ...
    context = await coordinator.begin_tick("world", "run", 0)
