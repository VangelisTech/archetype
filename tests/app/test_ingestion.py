...
"""Durable external facts (issue #274, B).
...
visibility is the unit of truth. Facts ride the commit-identity machinery:
a fact is visible iff its claim is COMPLETE.
"""
...
from archetype.app.facts import FactMeta, fact_payload_digest
...
async def _visible_facts(c, world, storage) -> list[dict]:
    df = await c.query_service.query_components(
        [FactMeta], str(world.world_id), str(world.run_id), storage
    )
    return df.to_pylist()


async def test_ingest_appends_visible_fact_with_external_key_on_rows(tmp_path):
    c = ServiceContainer()
    try:
        ...
        assert receipt.fact_entity_id < 0, "facts live in the metadata id band"
        assert receipt.payload_digest == fact_payload_digest([Reading(value=21.5, unit="C")])

        rows = await _visible_facts(c, world, storage)
        assert len(rows) == 1
        assert rows[0]["factmeta__external_id"] == "sensor-001:evt-1"
...
async def test_duplicate_ids_dedupe_and_conflicts_fail_loudly(tmp_path):
    ...
    try:
        ...
        first = await c.ingestion_service.ingest_fact(
            wid, [Reading(value=1.0)], external_id="e-1", producer="p"
        )
        again = await c.ingestion_service.ingest_fact(
            wid, [Reading(value=1.0)], external_id="e-1", producer="p"
        )
...
async def test_hundred_concurrent_identical_submissions_one_visible_fact(tmp_path):
    c = ServiceContainer()
    try:
        storage = _storage(tmp_path)
        world = await _world(c, storage)
        ...
        async def submit():
            return await c.ingestion_service.ingest_fact(
        ...
        assert len(tokens) == 1, "every caller converges on the one visible fact"
...
async def test_crash_between_append_and_complete_recovers_without_duplication(
    ...
    try:
        ...
        assert rows[0]["factmeta__external_id"] == "crashy"
    finally:
        await c.shutdown()
...
async def test_facts_never_trigger_processors_and_survive_steps(tmp_path):
    ...
    try:
        ...
        assert receipt.fact_entity_id not in seen_entities, (
            "facts are non-processable: the step loop never sees them"
        )
        # The fact did not multiply across ticks (no quadratic history).
        rows = await _visible_facts(c, world, storage)
        assert len(rows) == 1
    finally:
        await c.shutdown()
