...
async def test_sweep_failure_pause_and_redrive_graph_uses_server_backoff(tmp_path, monkeypatch):
    ...
    try:
        ...
        with pytest.raises(RecoverySweepStaleError, match="not live at fence"):
...
async def test_sweep_redrive_replay_requires_its_exact_durable_receipt(tmp_path, monkeypatch):
    now_ms = [2_500_000]
    monkeypatch.setattr(catalog_module, "_now_ms", lambda: now_ms[0])
    catalog = await _catalog_with_world(tmp_path)
    try:
        ...
        _, first_lease = await catalog.lease_recovery_sweep(
            "world-1", _KIND, "worker", lease_ms=100
        )
        ...
        # An ordinary leased -> idle yield has the same visible target shape
        # as a redrive response whose expected fence was one lower. It is not
        # evidence that that redrive committed.
        with pytest.raises(RecoverySweepStaleError, match="not paused at the expected fence"):
            await catalog.redrive_recovery_sweep(
                "world-1",
                _KIND,
        ...
        paused = await catalog.pause_recovery_sweep(
            ...
            error_code="handler_failed",
            error_detail="operator intervention required",
        )
        redriven = await catalog.redrive_recovery_sweep(
            "world-1", _KIND, expected_fence_epoch=paused.fence_epoch
        )
        await catalog.close()
        catalog = SqliteControlCatalog(tmp_path / "catalog.db")
        with pytest.raises(RecoverySweepStaleError, match="not paused at the expected fence"):
            await catalog.redrive_recovery_sweep(
                "world-1", _KIND, expected_fence_epoch=paused.fence_epoch, delay_ms=1
            )
        assert (
            await catalog.redrive_recovery_sweep(
                "world-1", _KIND, expected_fence_epoch=paused.fence_epoch
        ...
        # Once a later lease consumes the redriven idle row, its yield cannot
        # masquerade as a replay of another redrive.
...
async def test_sparse_exception_retry_dlq_redrive_resolve_and_exact_lookup(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert replay == first
        with pytest.raises(RecoveryExceptionConflictError, match="attempt count changed"):
...
async def test_exception_redrive_replay_requires_its_exact_durable_receipt(tmp_path, monkeypatch):
    now_ms = [3_500_000]
    monkeypatch.setattr(catalog_module, "_now_ms", lambda: now_ms[0])
    catalog = await _catalog_with_world(tmp_path)
    try:
        ...
        with pytest.raises(RecoveryExceptionConflictError, match="not dead-lettered"):
            await catalog.redrive_recovery_exception(
                "world-1",
                _KIND,
                "worker",
        ...
        later_retry = await catalog.retry_recovery_exception(
            ...
            error_code="handler_failed",
            error_detail="ordinary retry after redrive",
            retry_delay_ms=0,
            max_attempts=5,
        )
        assert later_retry.status == "retry_wait"
        assert later_retry.sweep_key == sweep.sweep_key
        with pytest.raises(RecoveryExceptionConflictError, match="not dead-lettered"):
            await catalog.redrive_recovery_exception(
                "world-1",
                _KIND,
