...
"""Bundle value contracts for durable sandbox-artifact finalization.
...
Everything here is a pure value type or protocol.  The catalog-bound
``PreparedArtifactBundleRequest`` stays in
``archetype.app.artifacts.bundle_models`` because its validation binds to
the control catalog's publication-key authority.
"""
...
# Iceberg snapshot ids are signed 64-bit; the same bound is pinned as
# archetype.app.limits.MAX_ICEBERG_SNAPSHOT_ID for application consumers.
_MAX_ICEBERG_SNAPSHOT_ID = (1 << 63) - 1
...
class ArtifactPublishReceipt(BaseModel):
    ...
    def _exact_snapshot_integer(cls, value: object) -> int:
        ...
        if value < 0 or value > _MAX_ICEBERG_SNAPSHOT_ID:
