...
"""
Archetype DSL — DataFrame-First Design
=======================================

An ergonomic DSL that honors the core engine by compiling to pure DataFrame transforms.
...
- Query API separate from behavior definition
...
Usage:
    ...
    class ConditionalDamage:
        ...
        def transform(self, arch, tick, dt):
            ...
            return {
                "health__current": when(arch.health.current < 50)
                    .then(50)
                    .otherwise(arch.health.current - 10)
            }
    
    async with World("sim") as world:
        world.register(Move)
        world.register(ConditionalDamage)
        await world.spawn(Position(x=0, y=0), Velocity(vx=1, vy=2), Health(current=100))
