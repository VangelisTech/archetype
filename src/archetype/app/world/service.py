...
class WorldRegistry:
    ...
    def insert(self, world: AsyncWorld) -> None:
        ...
        if wid in self._worlds and self._worlds[wid] is not world:
            raise ValueError(f"World with ID '{world.world_id}' already exists.")
        if world.name and world.name in self._names and self._names[world.name] != wid:
            raise ValueError(f"World with name '{world.name}' already exists.")
...
class WorldOrchestrator:
    ...
    live worlds). Does NOT depend on StorageService or CommandScheduler.
    """
...
class WorldService:
    ...
    def control_catalog(self, world_id: str | UUID):
        ...
        if record is None:
            raise KeyError(f"World with ID '{world_id}' has no registered storage identity.")
        return self._storage_service.get_control_catalog(record[0])
