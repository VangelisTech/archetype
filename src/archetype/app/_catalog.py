...
class ArtifactPublicationRecord:
    """Durable control-plane state for one artifact bundle.

    ``request_json`` is the credential-free replay input. ``records_json`` is
    written only after every portable object is durably uploaded, so a
    reconciler can finish the Iceberg index without reopening the sandbox.
    """
    ...
    idempotency_key: str
    request_digest: str
    status: str  # "PENDING" | "UPLOADED" | "INDEXED" | "EXPIRED"
    request_json: str
    records_json: str
    claimant: str
    lease_expires_at: float
