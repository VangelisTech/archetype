...
if TYPE_CHECKING:
    ...
    from archetype.core.hooks import AsyncHookHandler, FireMode, HookEvent, HookHandle
...
class iMutationService(Protocol):
    ...
    async def remove_processor(self, world_id: str | UUID, proc_type: type[iAsyncProcessor]) -> None: ...
...
class iSimulationService(Protocol):
    ...
    async def run(self, world_id: str | UUID, run_config: RunConfig, **input_kwargs) -> RunResult: ...
    async def run_episode(self, world_id: str | UUID, config: EpisodeConfig, **input_kwargs) -> EpisodeResult: ...
    async def run_rollout(self, world_id: str | UUID, config: RolloutConfig, **input_kwargs) -> RolloutResult: ...
...
class iQueryService(Protocol):
    ...
    async def list_signatures(self, storage_config: StorageConfig | None = None) -> list[ArchetypeSignature]: ...
...
class iCommandBroker(Protocol):
    ...
    async def dequeue_due(self, world_id: str | UUID, tick: int, limit: int | None = None) -> list[Command]: ...
...
class iCommandService(Protocol):
    ...
    async def create_entity(self, ctx: ActorCtx, world_id: str | UUID, components: list[Component]) -> int: ...
    async def remove_entity(self, ctx: ActorCtx, world_id: str | UUID, entity_id: int) -> None: ...
    async def update_entity(self, ctx: ActorCtx, world_id: str | UUID, entity_id: int, components: list[Component]) -> None: ...
    async def add_components(self, ctx: ActorCtx, world_id: str | UUID, entity_id: int, components: list[Component]) -> None: ...
    async def remove_components(self, ctx: ActorCtx, world_id: str | UUID, entity_id: int, component_types: list[type[Component]]) -> None: ...
    async def add_processor(self, ctx: ActorCtx, world_id: str | UUID, processor: iAsyncProcessor) -> None: ...
    async def remove_processor(self, ctx: ActorCtx, world_id: str | UUID, proc_type: type[iAsyncProcessor]) -> None: ...

    # ── Lifecycle (gated, direct) — returns WorldInfo, never iWorld ────────

    async def create_world(self, ctx: ActorCtx, config: WorldConfig, storage_config: StorageConfig | None = None, cache_config: CacheConfig | None = None) -> WorldInfo: ...
    async def fork_world(self, ctx: ActorCtx, source_world_id: str | UUID, name: str | None = None, *, storage_config: StorageConfig | None = None, cache_config: CacheConfig | None = None) -> WorldInfo: ...
    ...
    async def step(self, ctx: ActorCtx, world_id: str | UUID, run_config: RunConfig, **input_kwargs) -> None: ...
    async def run(self, ctx: ActorCtx, world_id: str | UUID, run_config: RunConfig, **input_kwargs) -> RunResult: ...
    async def run_episode(self, ctx: ActorCtx, world_id: str | UUID, config: EpisodeConfig, **input_kwargs) -> EpisodeResult: ...
    async def run_rollout(self, ctx: ActorCtx, world_id: str | UUID, config: RolloutConfig, **input_kwargs) -> RolloutResult: ...

    # ── Resource / hook attachment (gated) ─────────────────────────────────

    async def add_resource(self, ctx: ActorCtx, world_id: str | UUID, resource: object) -> None: ...
    async def add_hook(self, ctx: ActorCtx, world_id: str | UUID, event_type, fn, *, mode: str = "blocking"): ...
    ...
    async def get_audit_history(self, ctx: ActorCtx, world_id: str | UUID | None = None, *, tick_range: tuple[int, int] | None = None, actor_id: str | UUID | None = None, signer_address: str | None = None, idempotency_key: str | None = None, limit: int | None = None) -> DataFrame: ...

    # ── Queries (gated reads) ─────────────────────────────────────────────

    async def query_archetype(self, ctx: ActorCtx, sig: ArchetypeSignature, world_id: str, run_id: str, storage_config: StorageConfig | None = None, *, ticks: list[int] | None = None, entity_ids: list[int] | None = None, components: list[type[Component]] | None = None) -> DataFrame: ...
    async def list_signatures(self, ctx: ActorCtx, storage_config: StorageConfig | None = None) -> list[ArchetypeSignature]: ...

    # ── Tick-deferred path (queued) ───────────────────────────────────────

    async def submit(self, ctx: ActorCtx, world_id: str | UUID, cmd: Command) -> UUID: ...
    async def submit_batch(self, ctx: ActorCtx, world_id: str | UUID, cmds: list[Command]) -> list[UUID]: ...
    async def submit_spawn(self, ctx: ActorCtx, world_id: str | UUID, components: list[Component], *, tick: int = 0, priority: int = 0) -> int: ...
