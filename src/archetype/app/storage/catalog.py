...
class SqliteControlCatalog:
    ...
    def _connect_sync(self) -> sqlite3.Connection:
        ...
        # Recovery coordination first shipped as schema v9. Keep development
        # catalogs created by prerelease v9 builds forward-compatible while
        # the schema is still landing: these receipts are deliberately
        # internal and prove an exact lost-response redrive replay.
        recovery_sweep_columns = {
            str(row["name"])
            for row in conn.execute("PRAGMA table_info(fleet_recovery_sweeps)").fetchall()
        }
        if "redrive_from_fence_epoch" not in recovery_sweep_columns:
            conn.execute(
                "ALTER TABLE fleet_recovery_sweeps ADD COLUMN redrive_from_fence_epoch INTEGER"
            )
        if "redrive_delay_ms" not in recovery_sweep_columns:
            conn.execute("ALTER TABLE fleet_recovery_sweeps ADD COLUMN redrive_delay_ms INTEGER")
        recovery_exception_columns = {
            str(row["name"])
            for row in conn.execute("PRAGMA table_info(fleet_recovery_exceptions)").fetchall()
        }
        if "redrive_from_attempt_count" not in recovery_exception_columns:
            conn.execute(
                "ALTER TABLE fleet_recovery_exceptions "
                "ADD COLUMN redrive_from_attempt_count INTEGER"
            )
        if "redrive_retry_delay_ms" not in recovery_exception_columns:
            conn.execute(
                "ALTER TABLE fleet_recovery_exceptions ADD COLUMN redrive_retry_delay_ms INTEGER"
            )
        conn.commit()
        self._conn = conn
        return conn
        ...
        def _renew() -> RecoverySweepRecord:
            ...
            with conn:
                ...
                updated = conn.execute(
                    ...
                    "updated_at_ms=?, redrive_from_fence_epoch=NULL, redrive_delay_ms=NULL "
        ...
        def _checkpoint() -> RecoverySweepRecord:
            ...
            with conn:
                ...
                updated = conn.execute(
                    ...
                    "updated_at_ms=?, redrive_from_fence_epoch=NULL, redrive_delay_ms=NULL "
                    "WHERE sweep_key=? AND status=? AND claimant=? "
        ...
        def _yield() -> RecoverySweepRecord:
            ...
            with conn:
                ...
                if existing is not None and (
                    ...
                    and _local_recovery_int(
                        existing,
                        "next_due_at_ms",
                        minimum=0,
                        maximum=_MAX_PORTABLE_COUNTER,
                    )
                    - _local_recovery_int(
                        existing,
                        "updated_at_ms",
        ...
        *,
        ...
        def _redrive() -> RecoverySweepRecord:
            ...
            with conn:
                ...
                source_status = RecoverySweepTransitionGraph.state(row["status"])
                redrive_from_fence_epoch = _local_optional_recovery_int(
                    row,
                    "redrive_from_fence_epoch",
                    minimum=0,
                    maximum=_MAX_PORTABLE_COUNTER,
                )
                redrive_delay_ms = _local_optional_recovery_int(
                    row,
                    "redrive_delay_ms",
                    minimum=0,
                    maximum=_MAX_RECOVERY_DELAY_MS,
                )
                if (
                    row["status"] == "idle"
                    and int(row["fence_epoch"]) == expected_fence_epoch + 1
                    and redrive_from_fence_epoch == expected_fence_epoch
                    and redrive_delay_ms == delay_ms
                ):
                    return _recovery_sweep_from_row(row)
                ...
                updated = conn.execute(
                    ...
                    "redrive_from_fence_epoch=?, redrive_delay_ms=? "
            ...
            with conn:
                ...
                if row is not None:
                    ...
                    if (
                        ...
                        and row["redrive_from_attempt_count"] is None
                        and row["redrive_retry_delay_ms"] is None
                        and _local_recovery_int(
                            row,
                            "retry_at_ms",
                            minimum=0,
        ...
        *,
        ...
        def _redrive() -> RecoveryExceptionRecord:
            ...
            with conn:
                ...
                source_status = RecoveryExceptionTransitionGraph.state(row["status"])
                redrive_from_attempt_count = _local_optional_recovery_int(
                    row,
                    "redrive_from_attempt_count",
                    minimum=0,
                    maximum=_MAX_PORTABLE_COUNTER,
                )
                redrive_retry_delay_ms = _local_optional_recovery_int(
                    row,
                ...
                if (
                    source_status is RecoveryExceptionStatus.RETRY_WAIT
                    and redrive_from_attempt_count == expected_attempt_count
                    and redrive_retry_delay_ms == retry_delay_ms
                ):
                    return _recovery_exception_from_row(row)
                try:
                    target_status = RecoveryExceptionTransitionGraph.transition(
