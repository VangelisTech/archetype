...
def _now_ms() -> int:
    """Catalog-authoritative wall time for recovery leases and backoff."""
...
def _require_recovery_text(value: str, *, field: str, max_chars: int) -> str:
    if not isinstance(value, str):
        raise TypeError(f"{field} must be a string")
    if not value.strip():
...
def _require_recovery_kind(value: str) -> str:
    value = _require_recovery_text(value, field="recovery kind", max_chars=128)
...
def _require_recovery_fence(value: int, *, field: str = "fence_epoch") -> int:
    """Require an exact non-boolean fence portable through the Worker wire."""

    if type(value) is not int:
        raise TypeError(f"{field} must be an integer")
    if value < 0 or value > _MAX_RECOVERY_COUNTER:
        raise ValueError(f"{field} must be between 0 and {_MAX_RECOVERY_COUNTER}")
    return value
...
class SqliteControlCatalog:
    ...
    async def acquire_artifact_publication(
        ...
        *,
        ...
        """
        claimant = _require_recovery_text(
        ...
        if retry_not_after_ms is not None:
            retry_not_after_ms = _require_recovery_fence(
    ...
    async def lease_recovery_sweep(
        ...
        *,
        ...
        kind = _require_recovery_kind(kind)
        claimant = _require_recovery_text(claimant, field="recovery claimant", max_chars=1024)
        ...
        def _lease() -> tuple[str, RecoverySweepRecord]:
            ...
            with conn:
                ...
                if (
                    existing.fence_epoch >= _MAX_RECOVERY_COUNTER
                    or existing.cycle >= _MAX_RECOVERY_COUNTER
                ):
                    raise RecoverySweepConflictError(
    ...
    async def renew_recovery_sweep(
        ...
        *,
        ...
        kind = _require_recovery_kind(kind)
        claimant = _require_recovery_text(claimant, field="recovery claimant", max_chars=1024)
        fence_epoch = _require_recovery_fence(fence_epoch)
        lease_ms = _require_recovery_lease(lease_ms)
    ...
    async def redrive_recovery_sweep(
        ...
        *,
        ...
        kind = _require_recovery_kind(kind)
        expected_fence_epoch = _require_recovery_fence(
            expected_fence_epoch, field="expected_fence_epoch"
        )
        if expected_fence_epoch == _MAX_RECOVERY_COUNTER:
    ...
    async def retry_recovery_exception(
        ...
        *,
        ...
        if (
            ...
            or expected_attempt_count >= _MAX_RECOVERY_COUNTER
        ):
            raise ValueError(
...
def _artifact_publication_from_row(row: sqlite3.Row) -> ArtifactPublicationRecord:
    ...
    if (
        ...
        or raw_lease_expires_at > _MAX_RECOVERY_COUNTER
    ):
    ...
    if (
        ...
        or raw_retry_until_ms > _MAX_RECOVERY_COUNTER
    ):
    ...
    if (
        ...
        or raw_attempt_count > _MAX_RECOVERY_COUNTER
    ):
...
def _recover_artifact_publication_row(
    ...
    *,
    ...
    if existing.attempt_count >= _MAX_RECOVERY_COUNTER:
        raise ArtifactPublicationConflictError(
...
def _validate_recovery_error(error_code: str, error_detail: str) -> tuple[str, str]:
    error_code = _require_recovery_text(
        error_code,
...
def _recovery_sweep_from_row(row: sqlite3.Row) -> RecoverySweepRecord:
    ...
    cycle = _local_recovery_int(row, "cycle", minimum=0, maximum=_MAX_RECOVERY_COUNTER)
    fence_epoch = _local_recovery_int(row, "fence_epoch", minimum=0, maximum=_MAX_RECOVERY_COUNTER)
    return RecoverySweepRecord(
        ...
        lease_expires_at_ms=_local_recovery_int(
            row, "lease_expires_at_ms", minimum=0, maximum=_MAX_RECOVERY_COUNTER
        ),
        fence_epoch=fence_epoch,
        active_subject_key=str(row["active_subject_key"]),
        consecutive_failures=_local_recovery_int(
            row, "consecutive_failures", minimum=0, maximum=_MAX_RECOVERY_COUNTER
        ),
        ...
        next_due_at_ms=_local_recovery_int(
            row, "next_due_at_ms", minimum=0, maximum=_MAX_RECOVERY_COUNTER
        ),
        ...
        created_at_ms=_local_recovery_int(
            row, "created_at_ms", minimum=0, maximum=_MAX_RECOVERY_COUNTER
        ),
        updated_at_ms=_local_recovery_int(
            row, "updated_at_ms", minimum=0, maximum=_MAX_RECOVERY_COUNTER
        ),
        paused_at_ms=_local_optional_recovery_int(
            row, "paused_at_ms", minimum=0, maximum=_MAX_RECOVERY_COUNTER
        ),
    )
...
def _recovery_exception_from_row(row: sqlite3.Row) -> RecoveryExceptionRecord:
    ...
    attempt_count = _local_recovery_int(
        row, "attempt_count", minimum=0, maximum=_MAX_RECOVERY_COUNTER
    )
    return RecoveryExceptionRecord(
        ...
        retry_at_ms=_local_recovery_int(
            row, "retry_at_ms", minimum=0, maximum=_MAX_RECOVERY_COUNTER
        ),
        last_error_code=_local_recovery_error_code(row),
        last_error_detail=str(row["last_error_detail"]),
        created_at_ms=_local_recovery_int(
        ...
        resolved_at_ms=_local_optional_recovery_int(
            row, "resolved_at_ms", minimum=0, maximum=_MAX_RECOVERY_COUNTER
        ),
        dead_lettered_at_ms=_local_optional_recovery_int(
            row, "dead_lettered_at_ms", minimum=0, maximum=_MAX_RECOVERY_COUNTER
        ),
    )
