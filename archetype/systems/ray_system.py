...
class _SnapshotQueryManager:
    """
    A proxy QueryManager that reads component data from a provided snapshot dictionary.
        ...
        if df is not None:
            ...
            if schema:
                ...
                try:
                    ...
                    print(f"ERROR (_SnapshotQueryManager): Failed to create empty Daft DataFrame for {component_type.__name__}: {e}")
                    raise e # Re-raise
            else:
                # This indicates an issue - the processor is querying a type
                # that wasn't even known/snapshotted at the start of the step.
                raise ValueError(f"SnapshotQueryManager: Schema not found for component type {component_type.__name__}. Was it registered?")

    def get_components(self, *component_types: Type[Component]) -> DataFrame:
        """Builds a join plan based on the snapshot DataFrames."""
        # This mirrors the logic in EcsQueryManager but uses self.get_component
    ...
    def component_for_entity(self, entity_id: int, component_type: Type[_C]) -> Optional[_C]:
        ...
        try:
            ...
            print(f"ERROR (_SnapshotQueryManager): Failed reconstructing {component_type.__name__} from snapshot for entity {entity_id}: {e}")
            return None
...
class _RemoteProcessorWrapper:
    ...
    def process_wrapper(self,
        ...
        proxy_querier = _SnapshotQueryManager(snapshot, component_schemas)
...
class RayDagSystem(System):
    ...
    async def execute(self, querier: EcsQueryManager, updater: EcsUpdateManager, dt: float, *args: Any, **kwargs: Any) -> None:
        """
